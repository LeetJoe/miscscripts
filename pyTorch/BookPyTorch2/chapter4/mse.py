...
# todo linalg: linear algebra 是 numpy 的线性函数库, inv 是取倒数操作
AA = np.linalg.inv(A.T.dot(A))

# todo l 是 AT*A 倒数，再乘上 AT*C，是一个实数
l=AA.dot(B)
P=A.dot(l)

x=np.linspace(-2,2,10) # todo 线性插值
x.shape=(1,10) # 一维向量转成二维

# todo A.shape=(2, 1), x.shape=(1, 10)
xx=A.dot(x)
