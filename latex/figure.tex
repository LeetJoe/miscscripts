\documentclass[a4paper]{article}
\usepackage{pgfplots}
\usepackage{pgfplotstable}
\pgfplotsset{width=7cm,compat=1.13}
\begin{document}
	
	





	\begin{tikzpicture}
		\begin{axis}
			[
			legend pos=outer north east,  % 将图例放在图外，位于图的东北角
			xlabel=iterations of EM,
			title=Number of Augmented Data,
			ymax=8500,
			ymin=0,
			xmin=-0.5,
			xmax=2.5,
			x tick label style={/pgf/number format/precision=10},
			nodes near coords={\pgfmathprintnumber{\pgfkeysvalueof{/data point/y}}},
			]
			\addplot
			table                               % 绘制原始数据的折线图
			{           		                % X，Y的原始数据
				X Y
				0 7047
				1 837
				2 218
			};
		\end{axis}
	\end{tikzpicture}


	\begin{tikzpicture}
	\begin{axis}
		[
		legend pos=outer north east,  % 将图例放在图外，位于图的东北角
		xlabel=iteration of EM,
		title=Result of Experiment(MRR),
		ymax=0.72,
		ymin=0.69,
		xmin=-0.5,
		xmax=4.5,
		%设置 y 坐标标注数字格式
		y tick label style={
			/pgf/number format/fixed zerofill,
			/pgf/number format/precision=2
		},
		nodes near coords={\pgfmathprintnumber{\pgfkeysvalueof{/data point/y}}},
		% 设置曲线注数字格式
		every node near coord/.append style={
			/pgf/number format/fixed zerofill,
			/pgf/number format/precision=4
		},
		]
		\addplot
		table                               % 绘制原始数据的折线图
		{           		                % X，Y的原始数据
			X Y
			0 0.6945787738806041
			1 0.7036995624772716
			2 0.7090021908599651
			3 0.7119454740505421
			4 0.7133672543999674
		};
		\addlegendentry{MRR}          % 给第一个图像添加图例
	\end{axis}
	\end{tikzpicture}
	
	
	\begin{tikzpicture}
		\begin{axis}
			[
				legend pos=outer north east,
				xlabel=iterations of EM,
				title=Validate Result of KGE on Test Data,
				ymax=65,
				ymin=15,
				xmin=-0.5,
				xmax=2.5,
				nodes near coords={\pgfmathprintnumber{\pgfkeysvalueof{/data point/y}}\%},
			]
			\addplot 
			table                               % 绘制原始数据的折线图
			{           		                % X，Y的原始数据
				X Y
				0 25.0264
				1 24.7693
				2 24.5499
			};
			\addplot
			table
			% table[y={create col/linear regression={y=Y}}] % 对输入的数据作线性回归
			{   				
				X Y
				0 39.2603
				1 38.4737
				2 38.3829
			};
			\addplot
			table
			{   				
				X Y
				0 53.2597
				1 52.9950
				2 52.9723
			};
			\addlegendentry{Hit@1}          % 给第一个图像添加图例，即原始函数y(x)
			\addlegendentry{Hit@3}          % 给第一个图像添加图例，即原始函数y(x)
			\addlegendentry{Hit@10}          % 给第一个图像添加图例，即原始函数y(x)

			% \addlegendentry{                 % 给第二个图像添加图例，即线性回归结果a*x+b
			% 	$\pgfmathprintnumber{\pgfplotstableregressiona} \cdot x
			% 	\pgfmathprintnumber[print sign]{\pgfplotstableregressionb}$}
		\end{axis}
	\end{tikzpicture}


	
	
	
	\begin{tikzpicture}
		\begin{axis}
			[
			legend pos=outer north east,
			title=Final Result on Test Data,
			xlabel=iterations of EM,
			ymax=65,
			ymin=15,
			xmin=-0.5,
			xmax=2.5,
			nodes near coords={\pgfmathprintnumber{\pgfkeysvalueof{/data point/y}}\%},
			]
			\addplot 
			table                               % 绘制原始数据的折线图
			{           		                % X，Y的原始数据
				X Y
				0 26.1382
				1 25.6315
				2 25.4802
			};
			\addplot
			table
			% table[y={create col/linear regression={y=Y}}] % 对输入的数据作线性回归
			{   				
				X Y
				0 40.0393
				1 39.1922
				2 39.1090
			};
			\addplot
			table
			{   				
				X Y
				0 53.6303
				1 53.3731
				2 53.3202
			};
			\addlegendentry{Hit@1}          % 给第一个图像添加图例，即原始函数y(x)
			\addlegendentry{Hit@3}          % 给第一个图像添加图例，即原始函数y(x)
			\addlegendentry{Hit@10}          % 给第一个图像添加图例，即原始函数y(x)
			
			% \addlegendentry{                 % 给第二个图像添加图例，即线性回归结果a*x+b
				% 	$\pgfmathprintnumber{\pgfplotstableregressiona} \cdot x
				% 	\pgfmathprintnumber[print sign]{\pgfplotstableregressionb}$}
		\end{axis}
	\end{tikzpicture}



\begin{figure}[htbp]
	\centering
	\begin{minipage}{0.24\textwidth}
		\centering
		\begin{tikzpicture}
			\begin{axis}[
				xmin=-0.5, xmax=5.5,
				ymin=0.697, ymax=0.723,
				axis lines=left,
				xmajorgrids=true,
				ymajorgrids=true,
				grid style={dashed},
				width=3in,
				height=2in,
				]
				\addplot[blue, mark=*] coordinates {
					(0, 0.6988438881047587)
					(1, 0.7140488498503678)
					(2, 0.7161419674672248)
					(3, 0.7166623096886564)
					(4, 0.7179786348444682)
					(5, 0.7184642948730695)
				};
			\end{axis}
		\end{tikzpicture}
	\end{minipage}
	\hfill
	\begin{minipage}{0.24\textwidth}
		\centering
		\begin{tikzpicture}
			\begin{axis}[
				xmin=-0.5, xmax=5.5,
				ymin=0.773, ymax=0.798,
				axis lines=left,
				xmajorgrids=true,
				ymajorgrids=true,
				grid style={dashed},
				width=3in,
				height=2in,
				]
				\addplot[blue, mark=*] coordinates {
					(0, 0.7743002152856375)
					(1, 0.7902326970581159)
					(2, 0.7912230574378458)
					(3, 0.7936997031491299)
					(4, 0.793597613618529)
					(5, 0.7940875571401176)
				};
			\end{axis}
		\end{tikzpicture}
	\end{minipage}
	\caption{MRR by iterations on ICEWS14 (left) and ICEWS05-15 (right).}
	\label{EM_Fig1}
\end{figure}


\end{document}
